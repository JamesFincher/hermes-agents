...
| `finance-captain` | `agents/finance-captain/` | Finance ops with snapshots, metric registry, figure provenance, tie-out gates, proposal-only writes; not a CPA; ships its own `fin` plugin. | Figure tokens | Tie-out + number fence + mutation fence |
